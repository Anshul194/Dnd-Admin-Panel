...
const BrandList: React.FC = () => {
  ...
  const { brands, loading, error, pagination, searchQuery } = useAppSelector(
    (state) => state.brand
  );
  ...
  const handleDeleteConfirm = async () => {
    if (categoryToDelete) {
      ...
      try {
        ...
        await dispatch(deleteBrand(categoryToDelete._id)).unwrap();

        setPopup({
          message: `Brand "${categoryToDelete.name}" deleted successfully`,
          type: "success",
          isVisible: true,
        });
        ...
        console.log(`Brand "${categoryToDelete.name}" deleted successfully`);
      } catch (error) {
  ...
  return (
    <div>
      <PageMeta
        title="Brand List | TailAdmin"
        description="List of all brands in TailAdmin"
      />
